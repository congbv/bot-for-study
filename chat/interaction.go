...
func initIntrHandler(bot *api.BotAPI, cafeconf config.CafeConfig) error {
	log.Printf("initializing interaction handler")
...
func (i *intrHandler) handle(
	...
	log.Printf("endpoint, data := splitEndpointIntrData(reqdata) %s, %s \n", endpoint, data)
...
func (i *intrHandler) updateMessage(
	...
	for _, f := range []func(){
		...
		f()
		if err != nil {
			log.Printf("sending update: %+v", err)
		}
	}
...
func (i *intrHandler) prepareUpdateText(
	...
	log.Printf("messageId in set Text : %d \n", msgInfo.MessageID)
...
func (i *intrHandler) prepareUpdateKeyboard(
	...
	log.Printf("messageId in keyboard : %d \n", msgInfo.MessageID)
